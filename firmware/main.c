...
void render_tri(Vertex * a, Vertex * b, Vertex * c, uint32_t is_second_buf)
{
    ...
    if (dy2 < 0x10) return;
    uint32_t ratio  = ((uint32_t)dy1 << 16) / dy2;
    ...
    for (int i = 0; i < 7; i++) {
        ...
        ((uint32_t *)&d)[i] = (i == 3)
            ? (ai >> 8) * (ratio2 >> 8) + (ci >> 8) * (ratio >> 8)
            : (ai * ratio2 + ci * ratio) >> 16;
    }
    d.y = b->y;
    Vertex *pd = &d;
...
static q16 v3_len(Vec3 a) {
    uint32_t d = (uint32_t)v3_dot(a, a);
    if (d == 0) return 0;
    int s = 0;
    while (!(d & 0xC0000000u)) { d <<= 2; s += 2; }
    uint32_t r = isqrt32(d);
    int sh = 8 - (s >> 1);
...
static int pack_vertex(Vertex *out, Vec4 clip, uint32_t cr, uint32_t cg, uint32_t cb)
{
    ...
    q16 nx = qdiv(clip.x, clip.w);
    q16 ny = qdiv(clip.y, clip.w);
    q16 nz = qdiv(clip.z, clip.w);

    int32_t sx_q4 = ((nx * 5120) >> 16) + 5120;
    int32_t sy_q4 = 3840 - ((ny * 3840) >> 16);
    ...
    if (out->r > 0xFFFF) out->r = 0xFFFF;
    if (out->g > 0xFFFF) out->g = 0xFFFF;
    if (out->b > 0xFFFF) out->b = 0xFFFF;
    return 1;
}

typedef struct { Vec3 pos; uint32_t r, g, b; } WorldVertex;
...
void frame(uint16_t angle)
{
    ...
    draw_mesh(&vp, &octa, buf);
    while((Xil_In32(B)) != 0) {}

    while((Xil_In32(B + 8) >> 20) != buf) {}
